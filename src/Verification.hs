...
genVCs (TEAssert te) = do
  withPath "assert-term" $ withTopFalse $ genTermVCs te
...
genVCs (TELet s ty rhs body) = do
  let path = "let-" ++ s
  withPath path $ withPath "rhs" $ withTopFalse $ genVCs rhs
  localVar s ty $ do
...
exprToSMT ctx (TEFunCall name args) = do
  ...
  fn <- case lookup name (funcs ctx) of
